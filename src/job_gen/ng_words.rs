...
mod tests {
    ...
    // ---- 実データ誤検出計測 (2026-08-03) に基づく回帰テスト ----
    // 計測: 実求人CSV 16ファイル / 求人2,736件 / 667,616字。
    // 詳細は claudedocs/EXPRESSION_DICT_FP_MEASUREMENT_2026-08-03.md。

    #[test]
    fn 表現辞書_レビュー見逃し5表現を全件検出する() {
        // 上の「4件以上」テストより強い保証。命令形ルールの major 絞り込み後も
        // 5表現すべてが検出され続けることを固定する。
        let r = expr_rules();
        for p in [
            "ネイルを外せ",
            "本業への影響もゼロ",
            ...
            assert!(!r.detect(p).is_empty(), "検出されるべき: {p}");
        }
    }

    #[test]
    fn 表現辞書_ネイルを外せ_は身だしなみ語ルールで検出される() {
        // 命令形 standalone ルールから裸の「外せ」を外したため、この表現は
        // major=ネイル / minor=外せ のルール1本で拾う形になった。
        let v = expr_rules().detect("ネイルを外せ");
        assert!(
            v.iter()
                .any(|x| x.major.starts_with("ネイル") && x.minor.starts_with("外せ")),
            "身だしなみ語ルールで検出されるべき: {v:?}"
        );
    }

    #[test]
    fn 表現辞書_外せない_は誤検出しない() {
        // 実データで観測された誤検出 (indeed-2026-07-10.csv、2件)。
        // 命令形「外せ」ではなく可能形の否定「外せない」の語幹に一致していた。
        let v = expr_rules().detect("どうしても外せない用事や体調不良等 なにかあれば仰ってください");
        assert!(v.is_empty(), "誤検出があってはならない: {v:?}");
    }

    #[test]
    fn 表現辞書_外せるの可能形は誤検出しない() {
        // 辞書ドラフト時点で想定されていた誤検出パターン。
        for s in ["指輪が外せる方歓迎", "時計を外せます", "工具は外せず固定です"] {
            let v = expr_rules().detect(s);
            assert!(v.is_empty(), "誤検出があってはならない ({s}): {v:?}");
        }
    }

    #[test]
    fn 表現辞書_エ段命令形の仮定形は誤検出しない() {
        // 「従えば」「黙れば」「謝れば」は仮定形であって命令ではない。
        for s in ["上司の指示に従えば安心です", "黙れば済む話ではありません", "謝れば解決します"] {
            let v = expr_rules().detect(s);
            assert!(v.is_empty(), "誤検出があってはならない ({s}): {v:?}");
        }
    }
    ...
    #[test]
    fn 表現辞書_命令形は絞り込み後も検出される() {
        // ロ段命令形は単独一致のまま。エ段命令形は句点・感嘆符・引用の「と」で確定させる。
        for s in [
            "私語はやめろ",
            "私物は捨てろ",
            "ルールに従え。",
            ...
            let v = expr_rules().detect(s);
            assert!(!v.is_empty(), "検出されるべき ({s}): {v:?}");
        }
    }

    #[test]
    fn 表現辞書_残業なしは要出典確認として検出され続ける() {
        // 実データ検出の98.7%を占めるルール。設計上の常時ヒットであり、
        // 意図せず落ちていないことを固定する (扱いの見直しは team-lead 判断待ち)。
        let v = expr_rules().detect("日勤のみ・残業なしでプライベートも充実");
        assert!(
            v.iter().any(|x| x.reason.starts_with("要出典確認")),
            "要出典確認として検出されるべき: {v:?}"
        );
    }
