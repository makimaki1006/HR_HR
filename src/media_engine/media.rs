...
pub const MEDIA_MASTER: &[(&str, &str, &[&str], &str, &str)] = &[
    ...
    (
        "en_tenshoku",
        "エン転職",
        &["employment.en-japan.com", "en-japan.com"],
        "job_board",
        "general",
    ),
    (
    ...
    (
        "benesse_saiyo",
        "ベネッセスタイルケア採用サイト",
        &["saiyo.benesse-style-care.co.jp"],
        "corporate",
        "care",
    ),
];
...
mod tests {
    ...
    fn index() -> HashMap<String, MediaRow> {
        HashMap::from([
            ...
            (
                "job-medley.com".to_string(),
                row("job_medley", "ジョブメドレー", "medical_care"),
            ),
        ])
    }
