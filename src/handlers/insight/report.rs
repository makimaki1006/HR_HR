...
pub(crate) fn generate_chapter_narrative(
    ...
    match category {
        ...
        InsightCategory::ActionProposal => {
            ...
            if let Some(top) = insights.first() {
                let cost_hint = match top.id {
                    InsightId::Ap1 => "コストあり・高インパクト",
                    InsightId::Ap2 => "コストゼロ・即日実行可能",
                    InsightId::Ap3 => "低コスト・中インパクト",
                    _ => "優先度高",
                };
...
///
/// 2026-05-21 事故由来: 旧実装は `match insight.id.as_str() { ... _ => String::new() }` で
///   14 件の Insight ID 未登録 → 推奨アクション空表示。
/// 2026-05-22 改修: `Insight.id: String → InsightId` enum 化に伴い `_` アームを削除、
///   コンパイル時に exhaustive 検査される構造に変更。
///   新規 Insight 追加時は InsightId variant の追加 → 本 match arm の追加が **強制** される。
///
/// 注: engine_flow.rs 未実装の SwF04 / SwF10 も exhaustive のため枝を用意。
///   実装時に文言を書き換える前提でプレースホルダ文言を入れている。
pub(crate) fn generate_so_what(insight: &Insight) -> String {
    match insight.id {
        InsightId::Hs1 => "→ 欠員が慢性化しており、現行条件での充足は困難。給与・勤務条件の見直しか採用チャネル拡大が必要".into(),
        InsightId::Hs2 => "→ 給与水準が市場を下回っている。中央値への引き上げで応募数改善が見込める".into(),
        InsightId::Hs3 => "→ 求人情報の開示項目を増やすだけで応募率が改善する（コストゼロで実行可能）".into(),
        InsightId::Hs4 => "→ 求人原稿が実態の逼迫度を反映していない。急募キーワードや待遇の具体記載を追加すべき".into(),
        InsightId::Hs5 => "→ 少数事業者が市場を支配しており、賃金交渉力が偏っている".into(),
        InsightId::Hs6 => "→ 通勤圏が限られている。近隣都市への掲載エリア拡大が即効性あり".into(),
        InsightId::Fc1 => "→ 求人数の増減トレンドは今後の競争環境を左右する".into(),
        InsightId::Fc2 => "→ 最低賃金との乖離が拡大すると法令リスクが生じる".into(),
        InsightId::Fc3 => "→ 生産年齢人口の減少は構造的問題。短期的な対策では解決しない".into(),
        InsightId::Fc4 => "→ 求人の長期掲載が常態化すると採用コストが増大する".into(),
        InsightId::Rc1 => "→ 地域内での相対的な位置づけを把握し、改善すべき軸を特定する".into(),
        InsightId::Rc2 => "→ 給与格差は人材流出の直接的な原因になる".into(),
        InsightId::Rc3 => "→ 求人密度は競争の激しさを示す指標".into(),
        InsightId::Cz1 => "→ 通勤圏内の人口規模は潜在的な採用プールの上限を示す".into(),
        InsightId::Cz2 => "→ 周辺地域との給与差が人材の流出入を決定する".into(),
        InsightId::Cz3 => "→ 通勤圏全体の高齢化は中長期的な労働力減少に直結する".into(),
        InsightId::Cf1 => "→ 距離ベースのポテンシャルと実際の通勤フローの差が地理的障壁を示す".into(),
        InsightId::Cf2 => "→ 最大の流入元を掲載エリアに追加することで応募者プールが拡大する".into(),
        InsightId::Cf3 => "→ 地元就業率は地域の自給自足度を示す。低い場合は広域採用が必要".into(),
        InsightId::Ap1 => "→ 給与改善は最もインパクトが大きいが、コストも伴う".into(),
        InsightId::Ap2 => "→ 求人原稿の改善はコストゼロで即日実行可能".into(),
        InsightId::Ap3 => "→ 掲載エリア拡大は低コストで人材プールを広げられる".into(),
        // SSDSE-A 系 (engine.rs)
        InsightId::Ls1 => "→ 労働力プールに対する求人数が示す採用余力。需給バランスを踏まえ、競合密度から打ち手の優先度を決める".into(),
        InsightId::Ls2 => "→ 産業集中度が高い地域は、当該産業の景気変動が採用市場全体に直結する。リスク分散の観点で要注意".into(),
        InsightId::Hh1 => "→ 単独世帯比率が高い地域では転居採用が成立しやすい一方、生活インフラ・住居支援の訴求が応募決定要因に".into(),
        InsightId::Mf1 => "→ 医療従事者採用では供給密度ギャップが指標。供給不足地域は給与プレミアム形成の好機".into(),
        InsightId::In1 => "→ 産業構成のアンバランスは特定スキルへの需要集中を生む。当該産業の人材調達競争を見据えた採用戦略が必要".into(),
        InsightId::Ge1 => "→ 可住地密度は採用母集団の上限を規定。低密度地域では通勤圏拡大・リモート併用が必須".into(),
        // 人流系 (engine_flow.rs)
        InsightId::SwF01 => "→ 夜勤シフトを伴う求人は応募ハードルが高い。夜勤手当・送迎・仮眠施設の整備が応募率を左右する".into(),
        InsightId::SwF02 => "→ 休日営業業態では平日と異なる人材調達戦略が必要。学生・主婦・副業層の取り込みが鍵".into(),
        InsightId::SwF03 => "→ 居住地と勤務地が分離するベッドタウン構造では、通勤圏全体を対象にした採用設計が有効".into(),
        // SwF04: engine_flow.rs 未実装。recruitment_diag のホワイトリストに ID のみ存在。
        // 将来実装時に文言を書き換える。
        InsightId::SwF04 => "→ 求人密度と人流のギャップが大きいメッシュでは、出店・募集強化エリアとして検討の価値がある".into(),
        InsightId::SwF05 => "→ 観光・宿泊・飲食業の潜在採用需要が顕在化していない。繁忙期に向けた前倒し採用が機会".into(),
        InsightId::SwF06 => "→ 人流回復に伴うサービス業の採用需要再加速。早期確保が競合に対する優位性".into(),
        InsightId::SwF07 => "→ 主要流入元が限定的だと特定地域からの採用に依存。配信エリア追加で人材プールを多様化".into(),
        InsightId::SwF08 => "→ 昼間人口 > 夜間人口の地域は周辺からの就業者が多い。広域配信で応募者層を拡大できる".into(),
        InsightId::SwF09 => "→ 季節変動の大きな人流地域では、年間採用計画より繁忙期前の前倒し採用が有効".into(),
        // SwF10: engine_flow.rs 未実装。recruitment_diag のホワイトリストに ID のみ存在。
        InsightId::SwF10 => "→ 企業立地と人流ピーク時間のズレが大きい場合、求人の営業時間設計を見直す余地がある".into(),
    }
}
...
mod tests {
    ...
    fn mock_insight(
        ...
        use std::str::FromStr;
        Insight {
            id: InsightId::from_str(id).expect("unknown InsightId in mock_insight"),
            category,
            severity,
