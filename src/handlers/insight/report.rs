...
pub(crate) fn generate_so_what(insight: &Insight) -> String {
    match insight.id.as_str() {
        ...
        // 2026-05-21 追加: agent A2 監査で「14 件の Insight が _ => String::new() で
        //   空文字 fallback → 推奨アクションが空表示」を検出。SSDSE-A 系 + 人流系
        //   (engine.rs / engine_flow.rs) の Insight すべてに so what を実装。
        // SSDSE-A 系 (engine.rs)
        "LS-1" => "→ 労働力プールに対する求人数が示す採用余力。需給バランスを踏まえ、競合密度から打ち手の優先度を決める".into(),
        "LS-2" => "→ 産業集中度が高い地域は、当該産業の景気変動が採用市場全体に直結する。リスク分散の観点で要注意".into(),
        "HH-1" => "→ 単独世帯比率が高い地域では転居採用が成立しやすい一方、生活インフラ・住居支援の訴求が応募決定要因に".into(),
        "MF-1" => "→ 医療従事者採用では供給密度ギャップが指標。供給不足地域は給与プレミアム形成の好機".into(),
        "IN-1" => "→ 産業構成のアンバランスは特定スキルへの需要集中を生む。当該産業の人材調達競争を見据えた採用戦略が必要".into(),
        "GE-1" => "→ 可住地密度は採用母集団の上限を規定。低密度地域では通勤圏拡大・リモート併用が必須".into(),
        // 人流系 (engine_flow.rs)
        "SW-F01" => "→ 夜勤シフトを伴う求人は応募ハードルが高い。夜勤手当・送迎・仮眠施設の整備が応募率を左右する".into(),
        "SW-F02" => "→ 休日営業業態では平日と異なる人材調達戦略が必要。学生・主婦・副業層の取り込みが鍵".into(),
        "SW-F03" => "→ 居住地と勤務地が分離するベッドタウン構造では、通勤圏全体を対象にした採用設計が有効".into(),
        "SW-F05" => "→ 観光・宿泊・飲食業の潜在採用需要が顕在化していない。繁忙期に向けた前倒し採用が機会".into(),
        "SW-F06" => "→ 人流回復に伴うサービス業の採用需要再加速。早期確保が競合に対する優位性".into(),
        "SW-F07" => "→ 主要流入元が限定的だと特定地域からの採用に依存。配信エリア追加で人材プールを多様化".into(),
        "SW-F08" => "→ 昼間人口 > 夜間人口の地域は周辺からの就業者が多い。広域配信で応募者層を拡大できる".into(),
        "SW-F09" => "→ 季節変動の大きな人流地域では、年間採用計画より繁忙期前の前倒し採用が有効".into(),
        _ => String::new(),
    }
}
