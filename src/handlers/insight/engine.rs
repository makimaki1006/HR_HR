...
fn analyze_hiring_structure(ctx: &InsightContext) -> Vec<Insight> {
    ...
    // HS-1: 慢性的人材不足シグナル
    if let Some(insight) = hs1_chronic_shortage(ctx) { out.push(insight); }
    // HS-2: 給与競争力不足
    if let Some(insight) = hs2_salary_competitiveness(ctx) { out.push(insight); }
    // HS-3: 情報開示不足
    if let Some(insight) = hs3_transparency_gap(ctx) { out.push(insight); }
    // HS-4: テキスト温度と採用難の乖離
    if let Some(insight) = hs4_temperature_mismatch(ctx) { out.push(insight); }
    // HS-5: 雇用者集中（独占的市場）
    if let Some(insight) = hs5_monopsony(ctx) { out.push(insight); }
    // HS-6: 空間的ミスマッチ
    if let Some(insight) = hs6_spatial_mismatch(ctx) { out.push(insight); }
...
fn hs1_chronic_shortage(ctx: &InsightContext) -> Option<Insight> {
    ...
    let trend_text = if ts_rates.len() >= 3 {
        ...
        format!("過去3ヶ月: {:.1}% → {:.1}% → {:.1}%",
            recent[0]*100.0, recent[1]*100.0, recent[2]*100.0)
    } else {
        "時系列データなし".to_string()
    };
    ...
    Some(Insight {
        ...
        body: format!(
            "正社員の欠員率は{:.1}%{}。{}",
            vacancy_rate * 100.0,
            if chronic { "で、過去3ヶ月連続で高水準を維持しています" } else { "です" },
            trend_text
        ),
        evidence: vec![
            Evidence { metric: "欠員率".into(), value: vacancy_rate, unit: "%".into(),
...
fn hs2_salary_competitiveness(ctx: &InsightContext) -> Option<Insight> {
    ...
    Some(Insight {
        ...
        body: format!(
            ...
            local_avg, national_avg, comp_index * 100.0,
            if below_count > 0 { format!("最低賃金未満の求人が{}件あります。", below_count) }
            else { String::new() }
        ),
        evidence: vec![
            Evidence { metric: "競争力指数".into(), value: comp_index, unit: "".into(),
                context: format!("全国平均=1.0, 閾値{:.1}", SALARY_COMP_WARNING) },
            Evidence { metric: "平均月給".into(), value: local_avg, unit: "円".into(),
...
fn hs3_transparency_gap(ctx: &InsightContext) -> Option<Insight> {
    let trans_row = ctx.transparency.iter()
        .find(|r| get_str_ref(r, "emp_group") == "正社員")?;
    let avg_trans = get_f64(trans_row, "avg_transparency");
    if avg_trans >= TRANSPARENCY_WARNING { return None; }
    ...
    let severity = if avg_trans < TRANSPARENCY_CRITICAL { Severity::Critical } else { Severity::Warning };
    ...
    Some(Insight {
        ...
        body: format!(
            ...
            avg_trans * 100.0, worst_item, worst_rate * 100.0
        ),
        evidence: vec![
            Evidence { metric: "開示度".into(), value: avg_trans, unit: "%".into(),
                context: format!("閾値{:.0}%", TRANSPARENCY_WARNING * 100.0) },
            Evidence { metric: format!("{}開示率", worst_item), value: worst_rate, unit: "%".into(),
                context: "最低項目".into() },
        ],
...
fn hs4_temperature_mismatch(ctx: &InsightContext) -> Option<Insight> {
    let temp_row = ctx.temperature.iter()
        .find(|r| get_str_ref(r, "emp_group") == "正社員")?;
    ...
    Some(Insight {
        ...
        body: format!(
            ...
            vacancy_rate * 100.0, temperature, urgency_density
        ),
        evidence: vec![
            Evidence { metric: "欠員率".into(), value: vacancy_rate, unit: "%".into(),
                context: "高水準".into() },
            Evidence { metric: "テキスト温度".into(), value: temperature, unit: "".into(),
...
fn hs5_monopsony(ctx: &InsightContext) -> Option<Insight> {
    let mono_row = ctx.monopsony.iter()
        .find(|r| get_str_ref(r, "emp_group") == "正社員")?;
    let hhi = get_f64(mono_row, "hhi");
    let top1_share = get_f64(mono_row, "top1_share");

    if hhi < HHI_CRITICAL && top1_share < TOP1_SHARE_CRITICAL { return None; }

    let severity = if hhi >= HHI_CRITICAL { Severity::Critical } else { Severity::Warning };
    ...
    Some(Insight {
        ...
        body: format!(
            ...
            top1_share * 100.0, hhi
        ),
        evidence: vec![
            Evidence { metric: "HHI".into(), value: hhi, unit: "".into(),
                context: format!("閾値{}", HHI_CRITICAL) },
            Evidence { metric: "上位1社シェア".into(), value: top1_share, unit: "%".into(),
...
fn hs6_spatial_mismatch(ctx: &InsightContext) -> Option<Insight> {
    ...
    if isolation < ISOLATION_WARNING { return None; }

    // 昼夜間人口比を取得
    let daytime_ratio = ctx.ext_daytime_pop.first()
    ...
    Some(Insight {
        ...
        body,
        evidence: vec![
            Evidence { metric: "孤立スコア".into(), value: isolation, unit: "".into(),
...
fn analyze_forecast(ctx: &InsightContext) -> Vec<Insight> {
    let mut out = Vec::new();
    if let Some(i) = fc1_posting_trend(ctx) { out.push(i); }
    if let Some(i) = fc2_salary_pressure(ctx) { out.push(i); }
    if let Some(i) = fc3_population_outlook(ctx) { out.push(i); }
    if let Some(i) = fc4_fulfillment_worsening(ctx) { out.push(i); }
    out
}

/// FC-1: 求人量トレンド
fn fc1_posting_trend(ctx: &InsightContext) -> Option<Insight> {
    let values: Vec<f64> = ctx.ts_counts.iter()
        .filter(|r| get_str_ref(r, "emp_group") == "正社員")
    ...
    Some(Insight {
        ...
        body: format!(
            "過去{}ヶ月で求人数は月平均{:+.1}%のペースで{}しています。\
             現在{:.0}件 → 6ヶ月後の推定{:.0}件。",
            values.len(), slope * 100.0, direction, latest, forecast_6m
        ),
        evidence: vec![
            Evidence { metric: "月次変化率".into(), value: slope, unit: "%/月".into(),
...
fn fc2_salary_pressure(ctx: &InsightContext) -> Option<Insight> {
    let salary_vals: Vec<f64> = ctx.ts_salary.iter()
        .filter(|r| get_str_ref(r, "emp_group") == "正社員")
    ...
    // 最低賃金の推移
    let min_wage_vals: Vec<f64> = ctx.ext_min_wage.iter()
    ...
    let comparison = if salary_slope > wage_slope_monthly { "上回っています" }
        else if salary_slope < wage_slope_monthly * 0.5 { "大きく下回っています" }
        else { "ほぼ同水準です" };

    let severity = if salary_slope < wage_slope_monthly * 0.5 { Severity::Warning }
        else if salary_slope > wage_slope_monthly { Severity::Positive }
        else { Severity::Info };
...
fn fc3_population_outlook(ctx: &InsightContext) -> Option<Insight> {
    if ctx.ext_pyramid.is_empty() { return None; }
    ...
    for row in &ctx.ext_pyramid {
        ...
        match age {
            // 18区分対応
            "15-19"|"20-24"|"25-29"|"30-34"|"35-39"|"40-44"|"45-49"|"50-54"|"55-59"|"60-64" => working_age += pop,
            // 9区分フォールバック
    ...
    if working_age < 1.0 { return None; }
    let decline_rate = safe_divide(retiring_soon, working_age)?;

    // 転入転出
    let net_migration = ctx.ext_migration.first()
        .map(|r| {
    ...
    let severity = if decline_rate > 0.30 && net_migration < 0.0 { Severity::Critical }
        else if decline_rate > 0.25 { Severity::Warning }
        else { Severity::Info };

    let migration_text = if net_migration > 0.0 { "転入超過" }
        else if net_migration < 0.0 { "転出超過" }
        else { "均衡" };
    ...
    Some(Insight {
        ...
        body: format!(
            "生産年齢人口のうち{:.0}%が55歳以上で、10年以内に大量退職が見込まれます。\
             人口移動は{}（純移動{:+.0}人）。{}",
            decline_rate * 100.0, migration_text, net_migration,
            if decline_rate > 0.25 && net_migration < 0.0 {
                "人口流出と高齢化の二重圧力で、労働力の確保が困難になるリスクがあります。"
            } else { "" }
        ),
        evidence: vec![
            Evidence { metric: "退職予備率".into(), value: decline_rate, unit: "%".into(),
                context: "55歳以上/生産年齢人口".into() },
            Evidence { metric: "純移動数".into(), value: net_migration, unit: "人".into(),
...
fn fc4_fulfillment_worsening(ctx: &InsightContext) -> Option<Insight> {
    let listing_days: Vec<f64> = ctx.ts_fulfillment.iter()
        .filter(|r| get_str_ref(r, "emp_group") == "正社員")
        .map(|r| get_f64(r, "avg_listing_days"))
        .collect();
    let days_slope = linear_slope(&listing_days)?;

    let churn_rates: Vec<f64> = ctx.ts_tracking.iter()
        .filter(|r| get_str_ref(r, "emp_group") == "正社員")
    ...
    // 両方が悪化傾向の場合のみ発火
    if days_slope <= 0.0 && churn_slope <= 0.0 { return None; }

    let latest_days = listing_days.last().copied().unwrap_or(0.0);
    let latest_churn = churn_rates.last().copied().unwrap_or(0.0);

    let severity = if days_slope > 0.03 && churn_slope > 0.02 { Severity::Warning }
        else { Severity::Info };
    ...
    Some(Insight {
        ...
        body: format!(
            "求人の平均掲載日数{:.0}日（月次変化{:+.1}%）、離職率{:.1}%（月次変化{:+.1}%）。\
             充足が困難になるリスクがあります。",
            latest_days, days_slope * 100.0, latest_churn * 100.0, churn_slope * 100.0
        ),
        evidence: vec![
            Evidence { metric: "掲載日数変化率".into(), value: days_slope, unit: "%/月".into(),
                context: "増加=悪化".into() },
            Evidence { metric: "離職率変化率".into(), value: churn_slope, unit: "%/月".into(),
                context: "増加=悪化".into() },
        ],
...
fn analyze_regional_comparison(ctx: &InsightContext) -> Vec<Insight> {
    let mut out = Vec::new();
    if let Some(i) = rc1_benchmark_ranking(ctx) { out.push(i); }
    if let Some(i) = rc2_salary_gap(ctx) { out.push(i); }
    if let Some(i) = rc3_population_density_cross(ctx) { out.push(i); }
    out
}

/// RC-1: 総合ベンチマーク順位
fn rc1_benchmark_ranking(ctx: &InsightContext) -> Option<Insight> {
    if ctx.region_benchmark.is_empty() || ctx.muni.is_empty() { return None; }

    let my_row = ctx.region_benchmark.first()?;
    let composite = get_f64(my_row, "composite_benchmark");
    if composite <= 0.0 { return None; }

    let severity = if composite < 30.0 { Severity::Warning }
        else if composite > 70.0 { Severity::Positive }
        else { Severity::Info };
    ...
    Some(Insight {
        ...
        body: format!(
            "{}の総合ベンチマークスコアは{:.1}点です。{}",
            ctx.muni, composite,
            if composite < 30.0 { "県内で下位に位置しており、改善が必要です。" }
            else if composite > 70.0 { "県内で上位に位置しています。" }
            else { "県内で中位の水準です。" }
        ),
        evidence: vec![
            Evidence { metric: "ベンチマーク".into(), value: composite, unit: "点".into(),
...
fn rc2_salary_gap(ctx: &InsightContext) -> Option<Insight> {
    if ctx.cascade.is_empty() { return None; }

    let my_row = ctx.cascade.iter()
        .find(|r| get_str_ref(r, "emp_group") == "正社員")?;
    let local_salary = get_f64(my_row, "avg_salary_min");
    let local_holidays = get_f64(my_row, "avg_annual_holidays");

    if local_salary <= 0.0 { return None; }

    // 全国平均との比較（salary_compテーブルから）
    let comp_row = ctx.salary_comp.iter()
        .find(|r| get_str_ref(r, "emp_group") == "正社員");
    let national_avg = comp_row.map(|r| get_f64(r, "national_mean_min")).unwrap_or(0.0);

    if national_avg <= 0.0 { return None; }
    let diff = local_salary - national_avg;

    let severity = if diff < -20000.0 { Severity::Warning }
        else if diff > 10000.0 { Severity::Positive }
        else { Severity::Info };
    ...
    Some(Insight {
        ...
        evidence: vec![
            Evidence { metric: "平均月給".into(), value: local_salary, unit: "円".into(),
                context: format!("全国{:.0}円", national_avg) },
            Evidence { metric: "全国比差額".into(), value: diff, unit: "円".into(),
...
fn rc3_population_density_cross(ctx: &InsightContext) -> Option<Insight> {
    if ctx.ext_population.is_empty() { return None; }

    let pop_row = ctx.ext_population.first()?;
    let total_pop = get_f64(pop_row, "total_population");
    if total_pop < 100.0 { return None; }

    // 求人数合計
    let total_postings: f64 = ctx.vacancy.iter()
        .map(|r| get_f64(r, "total_count"))
        .sum();

    let density = safe_divide(total_postings * 1000.0, total_pop)?;

    let severity = if density > 50.0 { Severity::Warning }
        else if density < 5.0 { Severity::Positive }
        else { Severity::Info };
    ...
    Some(Insight {
        ...
        body: format!(
            ...
            total_pop, total_postings, density,
            if density > 50.0 { "求人密度が非常に高く、採用競争が激しい地域です。" }
            else if density < 5.0 { "求人密度が低く、比較的穏やかな採用環境です。" }
            else { "" }
        ),
        evidence: vec![
            Evidence { metric: "求人密度".into(), value: density, unit: "件/千人".into(),
                context: "人口対比".into() },
        ],
...
fn generate_action_proposals(ctx: &InsightContext, existing: &[Insight]) -> Vec<Insight> {
    ...
    if existing.iter().any(|i| i.id == "HS-2") {
        if let Some(i) = ap1_salary_improvement(ctx) { out.push(i); }
    }
    // AP-2: HS-3 or HS-4 が発火していれば求人原稿改善提案
    if existing.iter().any(|i| i.id == "HS-3" || i.id == "HS-4") {
        if let Some(i) = ap2_posting_improvement(ctx) { out.push(i); }
    }
    // AP-3: HS-6 が発火していれば採用エリア拡大提案
    if existing.iter().any(|i| i.id == "HS-6") {
        if let Some(i) = ap3_area_expansion(ctx) { out.push(i); }
    }
...
fn ap1_salary_improvement(ctx: &InsightContext) -> Option<Insight> {
    ...
    if national_median <= 0.0 || local_avg <= 0.0 { return None; }

    let increase = national_median - local_avg;
    if increase <= 0.0 { return None; }
    ...
    Some(Insight {
        ...
        evidence: vec![
            Evidence { metric: "必要増額".into(), value: increase, unit: "円/月".into(),
                context: "全国中央値到達".into() },
            Evidence { metric: "年間コスト増".into(), value: annual_cost, unit: "円/人".into(),
                context: "12ヶ月換算".into() },
        ],
...
fn ap2_posting_improvement(ctx: &InsightContext) -> Option<Insight> {
    ...
    let missing: Vec<&str> = items.iter()
    ...
    Some(Insight {
        ...
        evidence: vec![
            Evidence { metric: "未開示項目数".into(), value: missing.len() as f64, unit: "件".into(),
...
fn ap3_area_expansion(ctx: &InsightContext) -> Option<Insight> {
    ...
    if daytime_ratio >= 1.0 { return None; } // 昼間人口 > 夜間人口の都市部では不要
...
fn analyze_commute_zone(ctx: &InsightContext) -> Vec<Insight> {
    let mut out = Vec::new();
    if ctx.commute_zone_count == 0 && ctx.commute_inflow_total == 0 { return out; }
    // 距離ベース
    if ctx.commute_zone_count > 0 {
        if let Some(i) = cz1_population_distribution(ctx) { out.push(i); }
        if let Some(i) = cz2_salary_gap(ctx) { out.push(i); }
        if let Some(i) = cz3_aging_risk(ctx) { out.push(i); }
    }
    // 実フローベース（国勢調査OD）
    if ctx.commute_inflow_total > 0 {
        if let Some(i) = cf1_actual_commute_zone(ctx) { out.push(i); }
        if let Some(i) = cf2_inflow_targeting(ctx) { out.push(i); }
        if let Some(i) = cf3_self_commute_analysis(ctx) { out.push(i); }
    }
    out
}
...
fn cz1_population_distribution(ctx: &InsightContext) -> Option<Insight> {
    if ctx.commute_zone_total_pop <= 0 { return None; }
    let local_pop: i64 = ctx.ext_pyramid.iter()
        .map(|r| get_f64(r, "male_count") as i64 + get_f64(r, "female_count") as i64)
        .sum();
    if local_pop <= 0 { return None; }
    let local_share = local_pop as f64 / ctx.commute_zone_total_pop as f64;

    Some(Insight {
        id: "CZ-1".to_string(),
        category: InsightCategory::RegionalCompare,
        severity: if local_share < 0.05 { Severity::Positive } else { Severity::Info },
        ...
        body: format!(
            ...
            ctx.muni, local_share * 100.0,
            if local_share < 0.05 { "広域採用戦略が有効です。" } else { "" }
        ),
        evidence: vec![
            Evidence { metric: "通勤圏人口".into(), value: ctx.commute_zone_total_pop as f64, unit: "人".into(),
...
fn cz2_salary_gap(ctx: &InsightContext) -> Option<Insight> {
    let local_row = ctx.cascade.iter().find(|r| get_str_ref(r, "emp_group") == "正社員")?;
    let local_sal = get_f64(local_row, "avg_salary_min");
    let sm_row = ctx.spatial_mismatch.iter()
        .find(|r| get_str_ref(r, "emp_group") == "正社員")
        .or_else(|| ctx.spatial_mismatch.first())?;
    let acc_sal = get_f64(sm_row, "accessible_avg_salary_30km");
    if local_sal <= 0.0 || acc_sal <= 0.0 { return None; }
    let gap_pct = (local_sal - acc_sal) / acc_sal * 100.0;

    Some(Insight {
        id: "CZ-2".to_string(),
        category: InsightCategory::RegionalCompare,
        severity: if gap_pct < -10.0 { Severity::Warning } else if gap_pct > 5.0 { Severity::Positive } else { Severity::Info },
        title: "通勤圏内の給与格差".to_string(),
        body: format!(
            "地元月給({:.0}円) vs 圏内平均({:.0}円) = {:+.1}%。{}",
            local_sal, acc_sal, gap_pct,
            if gap_pct < -10.0 { "周辺に人材が流出するリスクあり。" } else if gap_pct > 5.0 { "人材を引き付けやすい環境。" } else { "" }
        ),
        evidence: vec![
            Evidence { metric: "給与差".into(), value: gap_pct, unit: "%".into(), context: "30km圏比".into() },
        ],
        related_tabs: vec!["analysis", "competitive"],
    })
}
...
fn cz3_aging_risk(ctx: &InsightContext) -> Option<Insight> {
    ...
    if rate < 0.20 { return None; }

    Some(Insight {
        id: "CZ-3".to_string(),
        category: InsightCategory::Forecast,
        severity: if rate > 0.30 { Severity::Warning } else { Severity::Info },
        title: "通勤圏の高齢化動向".to_string(),
        body: format!(
            "通勤圏高齢化率{:.1}%。生産年齢人口{}人。{}",
            rate * 100.0,
            super::super::helpers::format_number(ctx.commute_zone_working_age),
            if rate > 0.30 { "長期的な労働力減少が懸念されます。" } else { "" }
        ),
        evidence: vec![
            Evidence { metric: "高齢化率".into(), value: rate, unit: "%".into(), context: "通勤圏30km".into() },
        ],
...
fn cf1_actual_commute_zone(ctx: &InsightContext) -> Option<Insight> {
    if ctx.commute_zone_total_pop <= 0 || ctx.commute_inflow_total <= 0 { return None; }

    let actual_ratio = ctx.commute_inflow_total as f64 / ctx.commute_zone_total_pop as f64;
    let top3_text: Vec<String> = ctx.commute_inflow_top3.iter()
    ...
    Some(Insight {
        id: "CF-1".to_string(),
        category: InsightCategory::RegionalCompare,
        severity: if actual_ratio < 0.01 { Severity::Warning } else { Severity::Info },
        ...
        body: format!(
            ...
            actual_ratio * 100.0,
            if top3_text.is_empty() { "データなし".to_string() } else { top3_text.join("、") },
            if actual_ratio < 0.01 { "距離ポテンシャルと実態の乖離が大きく、地理的障壁の可能性。" } else { "" }
        ),
        evidence: vec![
            Evidence { metric: "実通勤流入".into(), value: ctx.commute_inflow_total as f64, unit: "人".into(),
                context: "国勢調査2020".into() },
            Evidence { metric: "距離圏人口比".into(), value: actual_ratio, unit: "%".into(),
...
fn cf2_inflow_targeting(ctx: &InsightContext) -> Option<Insight> {
    if ctx.commute_inflow_top3.is_empty() { return None; }
...
fn cf3_self_commute_analysis(ctx: &InsightContext) -> Option<Insight> {
    if ctx.commute_self_rate <= 0.0 { return None; }

    let severity = if ctx.commute_self_rate > 0.7 { Severity::Positive }
        else if ctx.commute_self_rate < 0.3 { Severity::Warning }
        else { Severity::Info };
    ...
    Some(Insight {
        ...
        body: format!(
            ...
            if ctx.commute_self_rate < 0.3 { "地元就業率が低く、多くの住民が他地域に流出。" }
            else if ctx.commute_self_rate > 0.7 { "地元就業率が高く、地域内で労働力が循環。" }
            else { "" },
            super::super::helpers::format_number(ctx.commute_outflow_total),
        ),
        evidence: vec![
            Evidence { metric: "地元就業率".into(), value: ctx.commute_self_rate, unit: "%".into(),
                context: "国勢調査2020".into() },
            Evidence { metric: "通勤流出".into(), value: ctx.commute_outflow_total as f64, unit: "人".into(),
