...
fn normalize_text(text: &str) -> String {
    ...
    for ch in text.chars() {
        match ch {
            ...
            '．' => result.push('.'),
            // チルダ・ダッシュ類→統一。
            // 2026-08-03: カタカナ長音符 'ー'(U+30FC) は「セールス」「カレンダー」等の
            // 語中に普通に現れる。無条件に範囲記号へ変換すると、本文由来のテキストが
            // 語の途中で範囲分割され「491万6000円/30歳」を月給491万と誤読する事故が
            // 実測された。長音符は直前が数字・円・万のとき (「20ー30万」「25万円ー30万円」型)
            // に限って範囲記号とみなす。純粋なダッシュ類は従来どおり無条件変換。
            '～' | '〜' | '―' | '－' => result.push('~'),
            'ー' => {
                let range_context = result
                    .chars()
                    .last()
                    .is_some_and(|c| c.is_ascii_digit() || c == '円' || c == '万');
                if range_context {
                    result.push('~');
                } else {
                    result.push(ch);
...
mod tests {
    ...
    // =====================================================================
    // 2026-08-03: カタカナ長音符が範囲記号に化けて本文が誤分割された回帰テスト。
    //   実測: 説明文中の「(基本給+セールスインセンティブ...)」の「セー」の長音符が
    //   '~' に変換され、その位置で範囲分割 → 左側の「491万6000円/30歳」(年収実績) が
    //   月給4,916,000円と解釈された。長音符は直前が数字のときだけ範囲記号とみなす。
    // =====================================================================
    #[test]
    fn katakana_long_vowel_in_words_is_not_a_range_separator() {
        // 語中の長音符: 分割されず、数値は1つも拾えない (数字も円もない)
        let r = parse_salary("セールスインセンティブあり", SalaryType::Monthly);
        assert_eq!(r.min_value, None);
        // 円・万の直後の長音符: 従来どおり範囲記号として機能する
        let r = parse_salary("月給20万円ー30万円", SalaryType::Monthly);
        assert_eq!(r.min_value, Some(200_000));
        assert_eq!(r.max_value, Some(300_000));
        // 実測事故の縮約形: 長音符を含む語が範囲分割を誘発しない
        let r = parse_salary(
            "年間休日120日。491万6000円/30歳 <内訳> (基本給+セールスインセンティブ+残業手当)×12ヶ月",
            SalaryType::Monthly,
        );
        // 「491万6000円」自体は拾われるが、長音符での不正分割はされない
        // (この値の排除は score_salary の長さ上限と集計側の上限フィルタが担う)
        assert_eq!(r.min_value, Some(4_916_000));
    }
