...
pub(crate) fn render_mi_market_quadrant(
    ...
    html.push_str(&format!(
        "<p class=\"mi-note\" style=\"font-size:11px;color:#64748b;margin:0 0 8px;\">\
         ...
         サンプル少 (5 点未満) では中央値が偏りやすいため、各点の絶対値と下表の推奨アクションを主判断材料とし、\
         象限位置は補助的にご参照ください。<br/>\
...
fn render_mi_distribution_ranking(html: &mut String, scores: &[MunicipalityRecruitingScore]) {
    ...
    // Round 10 Phase 1B (2026-05-11): cap 到達グループ内 de-tie の注記 (簡潔版)
    html.push_str(
        "<p class=\"mi-note\" style=\"font-size:11px;color:#64748b;margin:0 0 8px;\">\
         ...
         <strong>同点表示の取り扱い</strong>: 母集団厚み指数 (thickness) が上限に到達する都市部自治体では、\
         同点表示を避けるため、上限到達グループ内の元順位で小さな差を付けています \
         (Round 10 Phase 1B / 2026-05-11)。上限未到達自治体の値は変わりません。<br/>\
