...
pub(super) fn build_fact_inventory(
    ...
    if agg.total_count > 0 {
        ...
        // 2026-07-22 数値監査対応: 勤務地内訳は給与を確認できた求人ベースの集計のため、
        // 総数 601 件と分母が異なる。基盤の違いを明記する (集計フレーム不整合の指摘)。
        push(
            "F-SIZE",
            "市場規模",
            format!(
                "重複整理後の求人は {} 件。掲載から間もない求人の割合 (目安) は {:.0}% (裏返すと約{:.0}%は掲載から時間が経った求人で、これが市場の常態)。件数の多い勤務地 (給与を確認できた求人ベース) は {}。{}",
    ...
    if let (Some(lo), Some(hi)) = (lo, hi) {
        ...
        push(
            ...
            "給与",
            format!(
                "下限給与の中央値 {} (n={}) / 上限給与の中央値 {} (それぞれ別々に集計した中央値で、その差は {:.0}万円。個々の求人の給与幅そのものではない。年俸表記の求人はこの集計から除外)。下限給与を確認できた求人のうち、上限も併記して幅を示すものはおよそ {:.0}%。",
                man_yen(lo),
    ...
    if let Some(c) = ctx {
        ...
        if c.commute_inflow_total > 0 || c.commute_outflow_total > 0 {
            ...
            let strategic_note = if c.commute_outflow_total > c.commute_inflow_total {
                "自然な読み方: 市外へ通っている地元居住者に「地元で同水準の条件で働ける (通勤時間の短縮)」を訴求する流れ。流入の多い自治体は配信対象の候補だが、同時に求人が集まる競合地でもある点に注意。"
            } else {
            ...
            push(
                ...
                "通勤",
                format!(
                    "市外へ通勤する人 {} 人 / 市外から来る人 {} 人 / 通勤者に占める市内完結の割合 {:.1}% (国勢調査 OD。全住民・全産業の通勤であり特定職種の求職者の動きそのものではない)。{}。周辺からの流入が多い地域は {}。{}",
    ...
    if let Some(pos) = &position {
        ...
        // 補足2: 中間値比較は単一値表記だと低めに出る旨 (集計基準も明記: 年俸月額換算込み)
        if let (Some(m), Some(p)) = (pos.market_median, pos.percentile_from_below) {
            s.push_str(&format!(
                " 下限と上限の中間値どうしの比較では下位から約 {:.0}% (市場の中間値の中央値 {}。年俸表記の求人を月額換算で含む基準) だが、幅を書かない単一値表記の求人は中間値=下限になるため、実態より低めに見える。",
                p,
                man_yen(m),
            ));
        }
...
fn build_card_statement(cb: &CardBrief, agg: &SurveyAggregation) -> Option<String> {
    ...
    if let Some(d) = cb.desc_salary_man {
        ...
        if field_best.map_or(false, |h| d * 10_000 > h) {
            parts.push(format!(
                "求人票の訴求文・説明文には月収{}万円の記載があるが、給与欄の上限には反映されていない",
                d
            ));
        }
    }
    ...
    if let Some(h) = cb.annual_holidays {
        ...
        if agg.jobbox.annual_holidays_values.len() >= 20 {
            if h >= 125 {
                s.push_str(&format!(
                    " (市場で125日以上を明示する求人は {:.0}% — この休日数が多い層に入る)",
