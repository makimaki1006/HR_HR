...
fn render_hiring_risk(html: &mut String, ctx: &CompanyContext) {
    ...
    // 「採用環境は良好/厳しい」等の断定を避け、「相対的に〜の傾向/可能性」表現に統一。
    // ※グレード判定は HW 求人テキスト指標ベースの相対評価であり、実採用結果との因果は別途検証要。
    let explanation = match ctx.hiring_risk_grade.as_str() {
        "A" => "採用環境が相対的に有利な可能性があります（HW指標ベース）。給与水準・地域特性面で人材確保に向く傾向がみられます。",
        "B" => "採用環境はおおむね中程度の可能性があり、一部の指標に注意の余地があります（HW指標ベース）。",
        "C" => "採用に中程度のリスクがある可能性があります。給与水準や地域の人口動態に改善余地がある可能性。",
        "D" => "採用リスクが相対的に高い可能性があります。複数の指標で不利な傾向がみられます（因果は別途検証要）。",
        _ => "採用環境が相対的に厳しい可能性があります。早急な対策の検討が望まれます（HW指標ベース）。",
    };
