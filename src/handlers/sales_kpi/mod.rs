...
//! データの出どころ
//! ------------------------------------------------------------------
//! 架電クオリティの GAS プロジェクト（Hubspot リポジトリ
//! `scripts\gas\call_quality_app\sales_kpi_sync.gs`）が毎朝スプレッドシートに
//! 書く7枚を読む。架電クオリティと同じスプレッドシートなので `SheetStore` を共有する。
//!
//! 同期を別プロジェクトにせず架電クオリティに同居させているのは、
//! HUBSPOT_TOKEN も取引を取る関数（`_tfFetchAll_`）も書き込み先も
//! 既にそちらに揃っているため。分けると同じトークンを2箇所で持つことになる。
//!
