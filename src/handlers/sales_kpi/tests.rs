...
#[test]
fn 週次の行が画面側の集計と一致する() {
    // 🔴 **両側に集計対象外が入っている前提で比べる**（2026-09-08 に揃えた）。
    // 週次シートを書く Python の `weekly_cells()` も、画面（ここ）も、同じ
    // `KPI営業_メンバー` の `集計対象` 列から同じ相手を落としている。
...
fn 週ベースの列は当月ベースと別の窓で数えている() {
    ...
    // 🔴 件数を直書きしない。集計対象外（2026-09-08）を入れた時点で 1,064 → 1,063 に
    //    動き、このテストが落ちた。見たいのは「月をまたぐと半分に見える」という
    //    関係であって、その日の件数ではない。
    //    参考: 2026-09-08 時点で W35=1,063 / W36=537。
    let (aug, sep) = (
        w35["totals"]["pool"].as_i64().unwrap(),
        w36["totals"]["pool"].as_i64().unwrap(),
    );
    assert!(
        aug > sep * 3 / 2,
        "当月ベースが月をまたいで大きく落ちていない（{aug} → {sep}）。\
         この落ち方こそが週次表に並べてはいけない理由なので、消えたら前提が変わっている"
    );
...
/// 「何時時点の数か」を画面に渡す。
///
/// 🔴 これが空だと、画面は当日の数を「集計中」としか言えない。
/// 取り直しは平日 6:30 と 18:00 の2回だけなので、その間は数字が止まっている。
/// 2026-09-08 に、朝 8:41 に取った 22件 が一日中出ていた（実数は 10,410件）。
#[test]
fn 架電をいつ取ったかを画面に渡す() {
    ...
    let body = with_meta("項目\t値\n架電の取得時刻\t2026-09-08 08:41\n");
    assert_eq!(
        body["calls"]["fetched_at"], "2026-09-08 08:41",
        "取得条件の「架電の取得時刻」がそのまま渡っていない"
    );

    // 取得条件に無いときは空文字。画面はこれを見て時刻の表示をやめる
    // （null にすると画面側で "null 時点" と出る事故が起きる）。
    let body = with_meta("項目\t値\n取得時刻\t2026-09-08 08:41\n");
    assert_eq!(
        body["calls"]["fetched_at"], "",
        "取得時刻が無いときは空文字にする"
    );
}
