...
pub const ALLOWED_SHEETS: &[&str] = &[
    ...
    "KPI営業_週次",
    // 2026-09-07 追加: 架電リストの担当者別。チーム別の数字が全社と合わないときに
    //   （担当者が入っていない取引が7千件あるため合わない）元の行を確かめられるようにする。
    "KPI営業_架電リスト_担当別",
    // 2026-09-11 追加: 決定者・決裁者の入力状況。画面は最新日とその1つ前しか出さないので、
    //   それ以前の日の行を確かめたいときはここから直接見る。
    "KPI営業_決定者",
    // 2026-09-21: コンサルダッシュボードが読む9枚
    "CS_取引",
    "CS_顧客",
    "CS_MTG",
    "CS_月次スナップショット",
    ...
    "CS_プロパティ履歴",
    "CS_メタ",
    // 2026-09-26 追加: 電話の要約（案件の詳細が読む）。要約の元の行を確かめられるようにする
    "CS_通話要約",
];
