...
pub const KNOWN_SHEETS: &[&str] = &[
    ...
    "求人応募_Deal健全性",
    "求人応募_データ品質",
    // 2026-08-17 追加: p8 の C-1 が読む運用シート（現場が手で編集する）。
    //   どちらの一覧にも載っておらず、乖離検出テストの網からも漏れていた。
    "アラート除外リスト",
    // 2026-08-17 追加: **タブが読んでいるのに、ここに載っていなかった7枚**。
    //   ソースから `SHEET_*` を抜いて突き合わせるテストで見つかった。
    //   従来の「KNOWN と ALLOWED を比べる」テストは、**どちらにも
    //   載っていないシートを捕まえられない**という穴があった。
    "月次明細",
    "都道府県月次",
    "メンバーマスタ",
    "BPOコーラー",
    ...
    //   `call_quality_app/sales_kpi_sync.gs` が毎朝書く。仕分け（実施/未実施/未処理）は
    //   ここには入っておらず、判定は Rust 側（handlers::sales_kpi）で行う。
    //   現場ヒアリングで判定が変わる見込みがあり、変わるたびにシートを
    //   作り直したくないため、シートには生に近い列だけを置いている。
    "KPI営業_商談",
    "KPI営業_アポ",
    "KPI営業_Cヨミ",
    "KPI営業_架電日次",
    ...
    "KPI営業_メンバー",
    "KPI営業_取得条件",
    // 2026-09-07 追加: 週に1行の記録。KPI営業_ で唯一、集計済みの値を持つ。
    //   HubSpot は今の状態しか返さないので、過去の週は材料からは作り直せない。
    //   書くのは Python 側（Hubspot リポジトリ `scripts/sales_kpi/sync_daily.py`）。
    "KPI営業_週次",
    // 2026-09-07 追加: 架電リストの内訳を担当者ごとに持つ。
    //   全社の合計しか出せなかった「架電リストの残り」を、画面のチーム／個人の
    //   絞り込みに連動させるために足した（現場要望）。
    "KPI営業_架電リスト_担当別",
    // 2026-09-11 追加: 決定者・決裁者の入力状況（日 × 担当者）。
    //   /sales-kpi の「決定者・決裁者」タブが読む。日付が違う行が積み上がるので、
    //   画面は最新日とその1つ前の日だけを切り出して使う。
    "KPI営業_決定者",
    // 2026-09-21: コンサルダッシュボード(/consulting)が読む9枚
    "CS_取引",
    "CS_顧客",
    "CS_MTG",
    "CS_月次スナップショット",
...
mod tests {
    ...
    fn 全シートが列挙されている() {
        ...
        // 3枚を取りこぼした（実際に一度間違えた）。
        //
        // 2026-08-17: 69 → 77。GAS の `buildSheetResponse_` を数えた 69枚に加えて、
        //   **Rust のタブが実際に読んでいるのに載っていなかった8枚**を足した
        //   （月次明細 / 都道府県月次 / メンバーマスタ / BPO系4枚 / アラート除外リスト）。
        //   GAS 側では別の経路で読んでいたため `buildSheetResponse_` の数え上げから
        //   漏れていた。**「GAS の呼び出し数」だけを正にすると取りこぼす**ことが分かった。
        //   実際に使われている名前はソースから抜いて突き合わせる別のテスト
        //   (`tabs::tests::タブが読むシートは両方の一覧に載っている`) が担保する。
        // 2026-09-05: 77 → 84。営業KPI(`/sales-kpi`)が読む7枚を足した
        //   （KPI営業_商談 / アポ / Cヨミ / 架電日次 / 架電リスト / メンバー / 取得条件）。
        //   これらは Python の日次同期が書くもので、GAS 版には存在しない。
