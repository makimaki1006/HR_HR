...
/// RARITY検索APIハンドラー（postingsテーブルベース）
pub async fn api_rarity(
    ...
    let db = match &state.hw_db {
        Some(db) => db,
        None => return Html(render_no_db_data("RARITY検索")),
    };

    let query_str = raw_query.0.unwrap_or_default();
    let params_map = parse_multi_value_query(&query_str);
    let employment_list = params_map.get("employment_type").cloned().unwrap_or_default();
    let salary_type_list = params_map.get("salary_type").cloned().unwrap_or_default();

    if employment_list.is_empty() && salary_type_list.is_empty() {
        return Html(
            r##"<p class="text-slate-500 text-sm">条件を選択して検索してください</p>"##
                .to_string(),
        );
    }
    ...
    if !employment_list.is_empty() {
        let placeholders: Vec<String> = employment_list
            .iter()
            .map(|_| {
                idx += 1;
                format!("?{}", idx)
            ...
            .collect();
        conditions.push(format!("employment_type IN ({})", placeholders.join(",")));
        bind_strings.extend(employment_list);
    }

    if !salary_type_list.is_empty() {
        let placeholders: Vec<String> = salary_type_list
            .iter()
            .map(|_| {
                idx += 1;
                format!("?{}", idx)
            ...
            .collect();
        conditions.push(format!("salary_type IN ({})", placeholders.join(",")));
        bind_strings.extend(salary_type_list);
    }

    let sql = format!(
        "SELECT employment_type, salary_type, COUNT(*) as cnt, AVG(NULLIF(salary_min,0)) as avg_min \
         FROM postings WHERE {} \
         GROUP BY employment_type, salary_type ORDER BY cnt DESC LIMIT 50",
        conditions.join(" AND ")
    );

    let bind_refs: Vec<&dyn rusqlite::types::ToSql> = bind_strings
        .iter()
        .map(|s| s as &dyn rusqlite::types::ToSql)
        .collect();
    let rows = match db.query(&sql, &bind_refs) {
        ...
        Err(e) => {
            ...
            return Html(
                r##"<p class="text-red-400 text-sm">検索に失敗しました</p>"##.to_string(),
            );
        }
    };
    ...
    if rows.is_empty() {
        return Html(
            r##"<p class="text-slate-500 text-sm">該当データがありません</p>"##.to_string(),
        );
    }

    let mut total_count: i64 = 0;
    let mut result_items: Vec<(String, String, i64, f64)> = Vec::new();

    for row in &rows {
        let emp = get_str(row, "employment_type");
        let st = get_str(row, "salary_type");
        let cnt = get_i64(row, "cnt");
        let avg = get_f64(row, "avg_min");
        total_count += cnt;
        result_items.push((emp, st, cnt, avg));
    }

    let mut html = format!(
        r##"<div class="flex gap-2 mb-2 flex-wrap">
    <span class="px-2 py-1 rounded text-xs text-white" style="background-color: #6366F1;">該当: {}件</span>
    ...
    for (emp, st, cnt, _avg) in &result_items {
    ...
    <span class="text-white font-semibold" style="font-size: 0.85rem; min-width: 80px;">{}</span>
    <span class="text-slate-400" style="font-size: 0.8rem; min-width: 60px;">{}</span>
    <span class="flex-1"></span>
    <span style="color: #6366F1; font-size: 0.85rem; font-weight: 500;">{}件</span>
