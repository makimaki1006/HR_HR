...
///
/// # なぜこのテストがいるのか
/// axum は同じパスに 2 つハンドラが登録されていると、**組み立ての時点で panic** する。
/// 起動しないので気づきそうに見えるが、実際には気づけなかった。
///
/// 2026-09-06、`.merge(handlers::indeed::router())` が `src/lib.rs` に 2 回
/// 入った状態が main にあった。営業KPI ブランチと main が同じ行を別の位置に
/// 持っていて、マージが両方を残したため。`cargo test` は全部通り、
/// `cargo build` も通る。壊れるのは**起動時だけ**。
///
/// 結果として、営業KPI の作業は main に入っていたのに一度もデプロイに成功して
/// おらず、本番はその手前のコミットで動き続けていた
/// （/health の commit が 3 コミット前を指していた）。
///
/// ルーターを 1 回組み立てるだけで、この種の事故は止まる。
#[test]
fn アプリのルーターが二重登録なしで組み立てられる() {
    let (_tmp, hw_db) = make_tmp_hw_db();
    let state = make_test_state(hw_db);
    // 二重登録があればこの行で panic する
    let _router = crate::build_app(state);
}
