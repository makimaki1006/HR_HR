...
pub fn build_phone(sheets: &Sheets, today: NaiveDate) -> Value {
    ...
    json!({
        ...
        "reach": {
            ...
            // 🔴 画面にそのまま出る文。英語の内部名（Call / Deal / 多対多）で書かない（2026-09-23 実機）
...
fn contact_cmp_json(
    ...
    v["not_causal"] = json!(
        "交代が接触を減らした・増やした証拠ではありません。危ない案件だから担当を替えた可能性もあり、向きは決まりません\
    （過去の検証で、同じ取引に偽の交代日を置いた比較対象と並べると、前の接触の量をそろえたところで差が消えました）。\
    ...
    まとめでは、同じ拠点・同じ交代日の行を1件の交代と数えています。表の行には、その交代の値をそのまま出しています"
    );
    v["dir_rule"] = json!(format!(
        "増えた・減ったは、{PER_DAYS}日あたりの値を丸めずに比べています（わずかな差でも増えた・減ったに入ります）。\
    大きさは「変化（後−前）」で見てください。担当者ごとのまとめは、比べられた交代だけで作り、同じ人の同じ交代は1件と数えています。\
    比べられた交代が {MIN_PERSON_N} 件未満の人には印を付け、図には出していません"
    ));
    v
}
...
pub(super) fn deal_rows(sheets: &Sheets, today: NaiveDate) -> (Vec<Value>, Value) {
    ...
    スコアや確率は出していません。契約開始時点で解約する契約としない契約を見分ける力は 0.583\
