"""
fetch_estat_resident_population.py
===================================

e-Stat 国勢調査 R2 人口等基本集計 (statdisp_id=0003445080) 取得スクリプト

目的: 政令市の区 (designated_ward) を含む市区町村別の常住人口・男女・年齢階級
      を取得し、`v2_external_population` と `_pyramid` に追加投入することで、
      F2 推定 (resident × estimated_beta) のカバレッジを 175 designated_ward
      まで拡張する。

実装ステータス: 全 6 モード実装済 (本格 fetch は appId 確定後のユーザー手動)

設計書: docs/SURVEY_MARKET_INTELLIGENCE_PHASE3_DESIGNATED_WARD_DATA_AUDIT.md
パターン元: scripts/fetch_estat_15_1.py (Worker A5/A6)

CLI:
  python scripts/fetch_estat_resident_population.py --dry-run
  $env:ESTAT_APP_ID = "..."
  python scripts/fetch_estat_resident_population.py --metadata-only
  python scripts/fetch_estat_resident_population.py --sample-only
  python scripts/fetch_estat_resident_population.py --fetch
  python scripts/fetch_estat_resident_population.py --merge
  python scripts/fetch_estat_resident_population.py --validate
"""
...
META_FILE = OUTPUT_DIR / "estat_resident_metadata.json"
SAMPLE_FILE = TEMP_DIR / "estat_resident_sample.json"
PROGRESS_FILE = OUTPUT_DIR / "estat_resident_progress.json"
MERGED_CSV = OUTPUT_DIR / "estat_resident_merged.csv"
...
# ページファイル prefix (15-1 とは別)
PAGE_FILE_PREFIX = "estat_resident_page_"
...
def _load_progress(path: Path, stats_data_id: str = DEFAULT_STATS_DATA_ID) -> dict[str, Any]:
    ...
    return {
        ...
        "next_position": 1,
        "completed_pages": 0,
        "total_estimated": None,  # metadata 取得後に更新
...
def _load_master_designated_wards() -> set[str]:
    """master DB から designated_ward の市区町村コード集合を読み込む (read-only)。"""
    if not MASTER_DB_PATH.exists():
        return set()
    try:
        conn = sqlite3.connect(f"file:{MASTER_DB_PATH}?mode=ro", uri=True)
        try:
            rows = conn.execute(
                "SELECT municipality_code FROM municipality_code_master "
        ...
        print(f"[warn] master DB designated_ward read failed: {e}")
        return set()


# --------------------------------------------------------------------------- #
# モード: dry-run / metadata-only / sample-only
# --------------------------------------------------------------------------- #
...
def fetch_all_pages(
    ...
    while True:
        ...
        page_path = output_dir / f"{PAGE_FILE_PREFIX}{page_num:03d}.json"
...
def merge_pages(
    ...
    page_files = sorted(pages_dir.glob(f"{PAGE_FILE_PREFIX}*.json"))
    if not page_files:
...
def validate_clean_csv(
    ...
    NG (errors): municipality_code 形式違反、全国/都道府県集約混入、PK 重複、
                 サンプル地域欠損、population 数値変換不能、負値、empty CSV
    WARN: row count 期待値乖離、prefecture 数、orphan 率、軸 distinct 不足、
          designated_ward カバレッジ不足、総数/不詳の混入痕跡
    """
    errors: list[str] = []
    warnings: list[str] = []
    ...
    if expected_total:
        ...
        # 想定: 1,896 muni × 男女 (2) × 年齢階級 (~17~21) ≒ 64,000-80,000 行
        # 余裕を持って 30,000-200,000 を soft レンジに
        SOFT_MIN, SOFT_MAX = 30_000, 200_000
    ...
    if total_pop < 50_000_000:
        warnings.append(
            f"sum population {total_pop:,} unusually low (< 50M, expected ~120M for resident pop)"
        )
    ...
    # 9. PK 重複 (occupation 軸なし)
    pk_cols = ["municipality_code", "gender", "age_class"]
    ...
    # 11. municipality_code distinct count (期待 1,896 ± 5%)
    n_muni = df["municipality_code"].nunique()
    print(f"[info] distinct municipality_code: {n_muni:,}")
    expected_muni = 1896
    muni_diff_rate = abs(n_muni - expected_muni) / expected_muni
    if muni_diff_rate > 0.05:
        warnings.append(
            f"distinct municipality count {n_muni} deviates from expected {expected_muni} by {muni_diff_rate:.1%}"
        )
    ...
    # 13. designated_ward カバレッジ (本スクリプトの主目的)
    designated_codes = _load_master_designated_wards()
    if designated_codes:
        csv_codes = set(df["municipality_code"])
        csv_designated = csv_codes & designated_codes
        coverage = len(csv_designated) / max(len(designated_codes), 1)
        print(
            f"[info] designated_ward coverage: "
            f"{len(csv_designated)}/{len(designated_codes)} ({coverage:.1%})"
        )
        if coverage < 0.95:
            warnings.append(
                f"designated_ward coverage {coverage:.1%} < 95% — F2 estimation may be incomplete"
            )
    else:
        print("[info] master designated_ward unavailable; coverage check skipped")
...
def run_merge(args: argparse.Namespace) -> int:
    ...
    if not any(TEMP_DIR.glob(f"{PAGE_FILE_PREFIX}*.json")):
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=f"e-Stat resident population (sid={DEFAULT_STATS_DATA_ID}) fetch script",
    )
