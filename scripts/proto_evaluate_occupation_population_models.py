...
# ============================================================
# Model F2 (industrial_anchor_city、2026-05-04 Worker F 追加)
# ============================================================
# F2 設計原則:
#   - 本社集積オフィス (港区/中央区/千代田区) は HQ 減衰を適用 (Model F と同じ)
#   - 本社+工場併設地域 (豊田市/浜松市/太田市/堺市/四日市市) は HQ 減衰をスキップ
#   - 4 条件 AND で「データ駆動」に判定 (手動 allowlist 禁止、ユーザー指示)
#
# 判定条件 (4 条件 AND):
#   COND_A: 経済センサス上の製造業従業者シェアが高い (mfg_share > THRESHOLD)
#   COND_B: 製造業事業所あたりの平均従業員数が大きい (大規模工場)
#   COND_C: 昼夜間人口比が極端に高くない (オフィス街除外)
#   COND_D: SalesNow 本社過剰度が中程度以下 (大規模本社集積を除外)
#
# 閾値の根拠:
#   - mfg_share 0.12: 全国平均 (~0.16) 周辺。川崎/北九州 (~0.13) を含めるため
#   - mfg_emp_per_est 20: 本社オフィスのみだと 10 前後、工場併設は 25+
#   - day_night_ratio 150: 港区 453/中央区 456/千代田区 1753 を除外。豊田市 113 < 150
#   - hq_excess_ratio_E 5.0: 港区 75/千代田区 237 を除外、豊田市 2.69 を許容

ANCHOR_MFG_SHARE_MIN = 0.12   # 製造業従業者シェア下限 (12% 超)
ANCHOR_EMP_PER_EST_MIN = 20.0 # 製造業 1事業所あたり従業員数 (本社のみは ~10、工場併設は 25+)
ANCHOR_DN_RATIO_MAX = 150.0   # 昼夜間人口比上限 (DB は %、150 = 1.5 倍。オフィス街除外)
ANCHOR_HQ_EXCESS_E_MAX = 5.0  # 製造業本社過剰度上限 (港区 75/中央区 48/千代田区 237 を除外)

# anchor city 専用の追加ブースト
# 4 条件 AND で True 判定された市区町村は HQ-with-factory 集積。
# F3 (産業構成) だけでは TOP 10 に届かない中規模工業都市 (豊田/浜松/太田/四日市)
# の現業職を更に押し上げる。手動 allowlist 禁止のため、データ依存ブースト式とする。
#
# anchor_boost = 1 + ANCHOR_BOOST_GAMMA * (mfg_share - ANCHOR_MFG_SHARE_MIN) * sqrt(emp_per_est / ANCHOR_EMP_PER_EST_MIN)
# 上限 ANCHOR_BOOST_CAP で過剰防止。豊田市 (35.7%/66.0) は最大ブースト、川崎 (12.6%/25.3) は弱ブースト。
ANCHOR_BOOST_GAMMA = 6.0      # mfg_share 1pp 上昇あたりブースト強度
ANCHOR_BOOST_CAP = 2.5        # ブースト上限 (豊田市等の極端集中向け)
...
# ============================================================
# Model F2: industrial_anchor_city 判定 + F6 分岐 (Worker F)
# ============================================================

def load_industry_establishments(data):
    """産業構成 CSV から establishments (事業所数) を市区町村×産業別に読み込む.

    load_industry_data() は employees_total のみ読み込んでいるため、F2 用に別途 establishments
    も読み込む。CSV を 2 回読むのは冗長だが、既存関数 (model_f まで) を変更しないための分離。

    Returns:
        industry_est: dict[(pref, muni)] -> dict[ind -> establishments]
    """
    code_to_pref_muni = {}
    INCLUDE_AREA_TYPES = {"aggregate_city", "municipality", "special_ward",
                          "aggregate_special_wards"}
    ...
    industry_est = defaultdict(lambda: defaultdict(float))
    with open(INDUSTRY_CSV, "r", encoding="utf-8", newline="") as f:
        reader = csv.DictReader(f)
        for row in reader:
            ind = row["industry_code"]
            if ind not in TARGET_INDUSTRY_CODES:
                continue
            try:
                est = float(row["establishments"]) if row["establishments"] else 0.0
            except (ValueError, TypeError):
                est = 0.0
            if est <= 0:
                continue
    ...
    return {pm: dict(d) for pm, d in industry_est.items()}


def compute_industrial_anchor_cities(data, industry_emp, industry_est, sn_emp, national_emp):
    """4 条件 AND で industrial_anchor_city を判定.

    本社+工場併設都市 (例: 豊田市) と本社集積オフィス都市 (例: 港区) を区別する。
    手動 allowlist 禁止 (ユーザー指示)、すべてデータ駆動の閾値判定。

    判定条件 (4 条件 AND):
      COND_A: 経済センサス上の製造業従業者シェアが高い
              (industry_emp[muni, 'E'] / sum(industry_emp[muni])) > ANCHOR_MFG_SHARE_MIN
      COND_B: 製造業事業所あたりの平均従業員数が大きい (大規模工場)
              (industry_emp[muni, 'E'] / industry_est[muni, 'E']) > ANCHOR_EMP_PER_EST_MIN
      COND_C: 昼夜間人口比が極端に高くない (オフィス街除外)
              (daytime[muni].ratio) < ANCHOR_DN_RATIO_MAX  ※DB は % 表記
      COND_D: SalesNow 本社過剰度 (製造業 E) が中程度以下
              (sn_intensity_E / estat_intensity_E) < ANCHOR_HQ_EXCESS_E_MAX

    Args:
        data:         load_data() の結果 (population, daytime 等)
        industry_emp: data["industry_emp"] と同等
        industry_est: load_industry_establishments() の結果
        sn_emp:       load_salesnow_aggregate() の結果
        national_emp: data["national_emp"] と同等

    Returns:
        anchor_cities: set[(pref, muni)] - True 判定された市区町村
        diagnostics: dict - 各市区町村の判定根拠 (主要都市のみ)
    """
    # 全国 SalesNow 製造業従業員総数 (E)
    national_sn_E = 0.0
    for (pref, muni, ind), emp in sn_emp.items():
        if ind == "E":
            national_sn_E += emp
    ...
    # all_munis: industry_emp と population の和集合
    ...
    for (pref, muni) in all_munis:
        ...
        cond_a = mfg_share > ANCHOR_MFG_SHARE_MIN

        # COND_B: 製造業事業所あたり平均従業員数
        e_est = ind_est.get("E", 0.0)
        emp_per_est = (e_emp / e_est) if e_est > 0 else 0.0
        cond_b = emp_per_est > ANCHOR_EMP_PER_EST_MIN

        # COND_C: 昼夜間人口比 (DB 値は %)
        d = data["daytime"].get((pref, muni))
        dn_ratio = (d["ratio"] if d else 100.0)  # データなしは中立 100
        cond_c = dn_ratio < ANCHOR_DN_RATIO_MAX

        # COND_D: SalesNow 本社過剰度 E
        ...
        if national_sn_E > 0 and e_emp > 0 and national_estat_E > 0:
            ...
            # estat 欠損だが SalesNow に本社あり → HQ 集積扱い (cond_d FAIL)
            # 23 区按分前の元データで判定する想定
            hq_excess = 999.0
        else:
            hq_excess = 0.0  # SalesNow 不在 or estat 不在 → 中立扱い (FAIL or 中立判断)
        cond_d = hq_excess < ANCHOR_HQ_EXCESS_E_MAX

        is_anchor = cond_a and cond_b and cond_c and cond_d
        if is_anchor:
            anchor_cities.add((pref, muni))

        # 主要都市は audit に保存
        watch_list = [
            ("東京都", "港区"), ("東京都", "中央区"), ("東京都", "千代田区"),
            ("東京都", "新宿区"), ("東京都", "世田谷区"),
            ("愛知県", "豊田市"), ("群馬県", "太田市"), ("神奈川県", "川崎市"),
            ("三重県", "四日市市"), ("静岡県", "浜松市"), ("福岡県", "北九州市"),
            ("神奈川県", "相模原市"), ("大阪府", "堺市"),
            ("神奈川県", "横浜市"), ("大阪府", "大阪市"), ("愛知県", "名古屋市"),
            ("神奈川県", "厚木市"),
        ]
        if (pref, muni) in watch_list:
            audit[f"{pref} {muni}"] = {
                "anchor": is_anchor,
                "mfg_share": round(mfg_share, 3),
                "emp_per_est": round(emp_per_est, 1),
                "dn_ratio": round(dn_ratio, 1),
                "hq_excess_E": round(hq_excess, 2),
                "COND_A": cond_a, "COND_B": cond_b, "COND_C": cond_c, "COND_D": cond_d,
            }

    return anchor_cities, audit


def compute_f6_factor_v2(sn_emp, industry_emp, industry_est, national_emp, weights,
                         anchor_cities, alpha=F6_ALPHA):
    """F6 残差補正係数 v2 (anchor city は HQ 減衰スキップ + HQ-with-factory boost).

    アルゴリズム (compute_f6_factor との差分):
      anchor_cities ∋ (pref, muni) のとき:
        - HQ 減衰 (per_occ_factor *= damping) をスキップ
        - 既存工場ブースト (factory_excess_ratio > 1.5) は適用
        - 追加ブースト (HQ-with-factory boost):
          anchor_boost = 1 + GAMMA * (mfg_share - SHARE_MIN) * sqrt(emp_per_est / EST_MIN)
          上限 ANCHOR_BOOST_CAP。製造業 weights * (boost-1) で現業職に効果配分。
        - 結果として F6_factor[anchor_city, occ] >= 1.0
      anchor_cities ∌ (pref, muni) のとき:
        - 既存 compute_f6_factor() と完全同一 (HQ 減衰 + 工場ブースト両方適用)

    Args:
        sn_emp:        SalesNow 集約 dict[(pref, muni, ind)] -> 本社従業員数
        industry_emp:  data["industry_emp"]
        industry_est:  load_industry_establishments() の結果
        national_emp:  data["national_emp"]
        weights:       重みマスタ dict[(ind, occ)] -> 産業内職業シェア
        anchor_cities: set[(pref, muni)] industrial_anchor_city True 判定済み
        alpha:         減衰強度

    Returns:
        f6_factor: dict[(pref, muni)] -> dict[occ -> 係数]
        diagnostics: dict
    """
    import math
    # ---- 1. 全国 SalesNow 本社従業員総数 ----
    ...
    # ---- 2-3. 各市区町村の hq_excess_ratio / factory_excess_ratio ----
    hq_excess_ratio = defaultdict(dict)
    factory_excess_ratio = defaultdict(dict)
    ...
    # ---- 4. F6 係数の算出 (anchor city は HQ 減衰をスキップ) ----
    f6_factor = {}
    skipped_hq_damping_count = 0
    for (pref, muni), ratio_dict in hq_excess_ratio.items():
        per_occ_factor = {occ: 1.0 for occ in NATIONAL_OCCUPATION_RATIO}
        is_anchor = (pref, muni) in anchor_cities

        # (a) 本社過剰減衰 — anchor city は スキップ
        if is_anchor:
            skipped_hq_damping_count += 1
            # HQ 減衰スキップ。F6 = 1.0 のまま (工場ブーストのみ後段で適用)
        else:
            # Model F と完全同一の HQ 減衰
            for ind, ratio in ratio_dict.items():
                ind_threshold = F6_HQ_EXCESS_THRESHOLD_BY_IND.get(ind, F6_HQ_EXCESS_THRESHOLD)
                if ratio > ind_threshold:
                    damping = 1.0 / (1.0 + alpha * (ratio - 1.0))
        ...
        # (b) 工場過剰ブースト — anchor city も非 anchor city も同一適用
        if F6_BETA > 0:
            factory_dict = factory_excess_ratio.get((pref, muni), {})
            for ind, f_ratio in factory_dict.items():
                if f_ratio > F6_FACTORY_BOOST_THRESHOLD:
        ...
        # (c) anchor city 専用追加ブースト (HQ-with-factory boost)
        # 4 条件 AND で True 判定された市区町村のみ。製造業 weights * (boost-1) を加算。
        # anchor 判定済み = 大規模工場併設 確実 → 現業職を構造的に押し上げる。
        if is_anchor:
            ind_emp = industry_emp.get((pref, muni), {})
            ind_est = industry_est.get((pref, muni), {})
            e_emp = ind_emp.get("E", 0.0)
            total_ind_emp = sum(v for v in ind_emp.values())
            ...
            if mfg_share_v > ANCHOR_MFG_SHARE_MIN and emp_per_est_v > 0:
                # mfg_share の超過分 × emp_per_est 平方根スケーリング
                ...
                # 製造業 (E) の weights を介して現業職に配分
                # blue_collar_share[E, occ] が大きい職業ほど大きくブースト
                for occ in F6_BLUE_COLLAR_OCCUPATIONS:
                    blue_collar_share = weights.get(("E", occ), 0.0)
                    boost_amount = blue_collar_share * (anchor_boost - 1.0)
                    per_occ_factor[occ] *= (1.0 + boost_amount)
    ...
    diagnostics = {
        ...
        "skipped_hq_damping_count": skipped_hq_damping_count,
        "f6_factor_samples": {},
    }
    for sample_pref, sample_muni in [
        ("東京都", "港区"), ("東京都", "千代田区"), ("東京都", "中央区"),
        ...
        if key in f6_factor:
            diagnostics["f6_factor_samples"][label] = {
                "is_anchor": key in anchor_cities,
                "f6_per_occ": {
                    occ: round(f6_factor[key].get(occ, 1.0), 3)
                    for occ in F6_BLUE_COLLAR_OCCUPATIONS
                }
...
def model_f2(data, pref_occ_pop, industry_share, national_share, weights, f6_factor_v2):
    """Model F2: Model F の改良版 (industrial_anchor_city 分岐).

    本社集積オフィス (港区/中央区/千代田区) と本社+工場併設地域 (豊田/浜松/太田/堺/四日市)
    を 4 条件 AND データ判定で区別し、後者は HQ 減衰をスキップする。

    F1〜F5 は Model F と完全同一。F6_factor_v2 のみ anchor city 用に作り直す。
    F6_factor_v2 が Model F と同じ計算になるよう設計済み (anchor 不該当市区町村は
    Model F と同じ係数)。
    ...
    for (pref, muni), occ_dict in baseline.items():
        ...
        # F6 v2 (anchor city は HQ 減衰スキップ済み)
        f6_per_occ = f6_factor_v2.get((pref, muni), {})
...
def main():
    ...
    # Model F2 (2026-05-04 最終ラウンド、Worker F、industrial_anchor_city 分岐)
    print("  - Model F2: F + industrial_anchor_city 判定 (本社+工場併設地域は HQ 減衰スキップ)")
    industry_est = load_industry_establishments(data)
    anchor_cities, anchor_audit = compute_industrial_anchor_cities(
        data, data["industry_emp"], industry_est, sn_emp, data["national_emp"]
    )
    print(f"    industrial_anchor_city: {len(anchor_cities)} 市区町村が True 判定")
    print(f"    主要候補の判定 (True 期待: 豊田/浜松/太田/堺/川崎/相模原/北九州):")
    for label, audit in anchor_audit.items():
        marker = "✅" if audit["anchor"] else "❌"
        print(f"      {marker} {label:<22} mfg_share={audit['mfg_share']:.3f}  "
              f"emp/est={audit['emp_per_est']:.1f}  dn_ratio={audit['dn_ratio']:.1f}  "
              f"hq_excess_E={audit['hq_excess_E']:.2f}  "
              f"[A={audit['COND_A']} B={audit['COND_B']} C={audit['COND_C']} D={audit['COND_D']}]")
    if sn_emp:
        f6_factor_v2, f6_diag_v2 = compute_f6_factor_v2(
            sn_emp,
            data["industry_emp"],
            industry_est,
            data["national_emp"],
        ...
        print(f"    F6_v2 計算済み: {len(f6_factor_v2):,} 市区町村, HQ 減衰スキップ {f6_diag_v2['skipped_hq_damping_count']} 件")
    else:
        f6_factor_v2 = {}
        f6_diag_v2 = {"anchor_city_count": len(anchor_cities), "skipped_hq_damping_count": 0,
                      "f6_factor_samples": {}}
    result_f2, _ = model_f2(data, pref_occ_pop, industry_share, national_share, weights, f6_factor_v2)
    ...
    eval_f2 = _evaluate_model("Model F2", result_f2)

    all_evals = [eval_a, eval_b, eval_c, eval_e, eval_e2, eval_e3, eval_e4, eval_f, eval_f2]
    ...
    # 工業都市の詳細順位 (E vs F vs F2 比較)
    print("\n=== 工業都市 生産工程順位の改善 (Model E vs E2/E3/E4/F/F2) ===")
    ...
    for cand_pref, cand_muni in [("愛知県", "豊田市"), ("群馬県", "太田市"),
        ...
        print(f"    {loc_str:<22} E={ranks['E']}  E2={ranks['E2']}  E3={ranks['E3']}  "
              f"E4={ranks['E4']}  F={ranks['F']}  F2={ranks['F2']}")

    # E2/E3/E4/F/F2 の TOP 20 (主要 4 職業のみ)
    ...
    model_f_top20 = {occ: ranking_top_n(result_f, occ, n=20)
                      for occ in LOGISTIC_MFG_CONSTRUCTION_OCCUPATIONS}
    model_f2_top20 = {occ: ranking_top_n(result_f2, occ, n=20)
                      for occ in LOGISTIC_MFG_CONSTRUCTION_OCCUPATIONS}
    ...
    # Model F2 の主要職業 TOP 10 を出力 (anchor city 効果確認)
    print("\n=== Model F2 (F + industrial_anchor_city): 主要職業 TOP 10 ===")
    for occ in LOGISTIC_MFG_CONSTRUCTION_OCCUPATIONS:
        print(f"\n  >>> {occ}")
        top = ranking_top_n(result_f2, occ, n=10)
        for i, (loc, v) in enumerate(top, 1):
            print(f"    {i:2}. {loc:<24} {int(v):>10,} 人")
    ...
    best_eval = max([eval_e, eval_e2, eval_e3, eval_e4, eval_f, eval_f2], key=_model_score)
    print(f"\n=== 最良モデル ===")
    ...
    print(f"\n=== 合格条件達成数 (5 条件中) ===")
    for ev in [eval_e, eval_e2, eval_e3, eval_e4, eval_f, eval_f2]:
        pc = _pass_count(ev)
        if pc == 5:
            badge = "A grade (合格)"
        elif pc == 4:
    ...
    # 商品化判定 (合格モデル選定)
    # A/B grade のみ商品化合格
    pass_models = [ev for ev in [eval_e, eval_e2, eval_e3, eval_e4, eval_f, eval_f2] if _pass_count(ev) >= 4]
    if pass_models:
        ...
        # 部分合格 (C grade) 中で最良
        c_models = [ev for ev in [eval_e, eval_e2, eval_e3, eval_e4, eval_f, eval_f2] if _pass_count(ev) == 3]
        if c_models:
    ...
    output = {
        ...
        # === Model F2 (Worker F, industrial_anchor_city 分岐) ===
        "model_f2_top20": model_f2_top20,
        "model_f2_diagnostics": f6_diag_v2,
        "model_f2_parameters": {
            "ANCHOR_MFG_SHARE_MIN": ANCHOR_MFG_SHARE_MIN,
            "ANCHOR_EMP_PER_EST_MIN": ANCHOR_EMP_PER_EST_MIN,
            "ANCHOR_DN_RATIO_MAX": ANCHOR_DN_RATIO_MAX,
            "ANCHOR_HQ_EXCESS_E_MAX": ANCHOR_HQ_EXCESS_E_MAX,
            "F6_ALPHA": F6_ALPHA,
            "F6_BETA": F6_BETA,
        },
        "industrial_anchor_cities": {
            "count": len(anchor_cities),
            "cities": sorted([f"{p} {m}" for (p, m) in anchor_cities]),
            "watch_list_audit": anchor_audit,
        },
        ...
        "multi_model_comparison": {
            ev["name"]: {
                ...
                "pass_count_5": _pass_count(ev) if (ev["name"].startswith("Model E") or ev["name"] in ("Model F", "Model F2")) else None,
            } for ev in all_evals
        },
        ...
        "limitations": [
            ...
            "F2 (industrial_anchor_city) は 4 条件 AND データ判定で本社+工場併設地域を識別",
    ...
    # ============================================================
    # 最終結論 (Worker D 改善ラウンド + Worker E + Worker F)
    # ============================================================
    print("\n" + "=" * 75)
    print("FINAL RECOMMENDATION (Worker D/E/F 全ラウンド)")
    print("=" * 75)
    print(f"  最終推奨モデル: {final_recommendation}")
    print(f"  商品化判定:    {product_verdict}")
    print(f"  E grade={eval_e['grade']}  E2 grade={eval_e2['grade']}  "
          f"E3 grade={eval_e3['grade']}  E4 grade={eval_e4['grade']}  "
          f"F grade={eval_f['grade']}  F2 grade={eval_f2['grade']}")
    print(f"  製造系工業都市数 (TOP 10): E={eval_e['manufacturing_count']}  E2={eval_e2['manufacturing_count']}  "
          ...
          f"F={eval_f['manufacturing_count']}  F2={eval_f2['manufacturing_count']} / 9")
    print(f"  物流系湾岸都市数 (TOP 10): E={eval_e['logistics_count']}  E2={eval_e2['logistics_count']}  "
          ...
          f"F={eval_f['logistics_count']}  F2={eval_f2['logistics_count']} / 7")
    print(f"  港区生産工程順位:        E={eval_e['port_minato_rank']}  E2={eval_e2['port_minato_rank']}  "
          ...
          f"F={eval_f['port_minato_rank']}  F2={eval_f2['port_minato_rank']}")
    print(f"  Jaccard 平均:           E={eval_e['jaccard']:.3f}  E2={eval_e2['jaccard']:.3f}  "
          f"E3={eval_e3['jaccard']:.3f}  E4={eval_e4['jaccard']:.3f}  "
          f"F={eval_f['jaccard']:.3f}  F2={eval_f2['jaccard']:.3f}")

    # ============================================================
    # Worker F 数値目標 5 条件チェックテーブル (E3 / F / F2 比較)
    # ============================================================
    print("\n" + "=" * 75)
    print("Worker F 数値目標 (合格条件) チェックテーブル — E3 vs F vs F2")
    print("=" * 75)
    targets = [
    ...
    print(f"  {'条件':<26} {'E3':<14} {'F':<14} {'F2':<14}")
    f_pass_count = 0
    f2_pass_count = 0
    target_audit = []
    for label, check, fmt in targets:
        ...
        f2_mark = "PASS" if check(eval_f2) else "FAIL"
        if check(eval_f):
            f_pass_count += 1
        if check(eval_f2):
            f2_pass_count += 1
        print(f"  {label:<26} {(e3_v + ' ' + e3_mark):<14} {(f_v + ' ' + f_mark):<14} "
              f"{(f2_v + ' ' + f2_mark):<14}")
        target_audit.append({
            "label": label,
            "model_e3": e3_v, "e3_pass": check(eval_e3),
            "model_f": f_v,  "f_pass":  check(eval_f),
            "model_f2": f2_v, "f2_pass": check(eval_f2),
        })

    if f2_pass_count == 5:
        worker_f_grade = "A-"
    elif f2_pass_count == 4:
        worker_f_grade = "B"
    elif f2_pass_count == 3:
        worker_f_grade = "C"
    else:
        worker_f_grade = "D"
    ...
    print(f"  Model F2 合格数: {f2_pass_count}/5 → estimate_grade = {worker_f_grade}")

    if f2_pass_count >= 5:
        print("  ✅ Model F2 商品化本実装承認 (Worker F 数値目標すべて達成、Grade A-)")
    elif f2_pass_count == 4:
        print("  ⚠️ Model F2 Grade B 達成。本実装着手は別途判断")
    else:
        print("  ❌ Model F2 不合格。職業人口推定は補助指数止まり、")
        print("     主軸を 通勤OD/求人需要/企業密度 に切替推奨")

    # JSON にも Worker E/F 結果を追記
        ...
        "targets": [
            {
                ...
                "e3_pass": t["e3_pass"],
                "f_pass": t["f_pass"],
            } for t in target_audit
        ],
    }
    output["worker_f_target_check"] = {
        "model_f2_pass_count_5": f2_pass_count,
        "estimate_grade": worker_f_grade,
        "targets": target_audit,
    }
    # E3/F/F2 三者比較を専用フィールドとして保存
    output["model_comparison_e3_f_f2"] = {
        t["label"]: {
            "model_e3":  t["model_e3"], "e3_pass":  t["e3_pass"],
            "model_f":   t["model_f"],  "f_pass":   t["f_pass"],
            "model_f2":  t["model_f2"], "f2_pass":  t["f2_pass"],
        } for t in target_audit
    }
    # 再書き出し
    with open(OUT_JSON, "w", encoding="utf-8") as f:
