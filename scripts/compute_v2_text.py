...
def verify(db):
    ...
    注: v2_text_temperature はcompute_v2_phase2.pyで計算されるため、ここでは検証のみ。
    """
    print("\n=== 検証 ===")
    tables = ["v2_text_quality", "v2_keyword_profile"]
    # v2_text_temperatureが存在すれば検証に含める
    has_temp = db.execute(
        "SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='v2_text_temperature'"
    ).fetchone()[0]
    if has_temp:
        tables.append("v2_text_temperature")
    for table in tables:
...
def main():
    ...
    try:
        ...
        # 注: v2_text_temperature はcompute_v2_phase2.pyで一元管理（二重定義解消）
