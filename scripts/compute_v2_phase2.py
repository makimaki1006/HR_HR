...
def compute_l1_text_temperature(db):
    ...
    db.execute("""
        CREATE TABLE v2_text_temperature (
            ...
            avg_urgency_density REAL NOT NULL,
            avg_selectivity_density REAL NOT NULL,
    ...
    # 集計辞書: key → [temperatures, urgency_densities, selectivity_densities]
    stats = defaultdict(lambda: {"temps": [], "urg": [], "sel": []})

    for pref, muni, industry, et, text in rows:
        grp = emp_group(et)
        muni = muni or ""
        ...
        keys = [
            (pref, "", "", grp),              # 都道府県集計
            (pref, muni, "", grp) if muni else None,  # 市区町村集計
            (pref, "", industry, grp) if industry else None,  # 産業別集計
        ]
        for key in keys:
            if key is None:
                continue
            stats[key]["temps"].append(temp)
            stats[key]["urg"].append(urg_density)
            stats[key]["sel"].append(sel_density)
    ...
    db.executemany("""
        ...
        (prefecture, municipality, industry_raw, emp_group,
         total_count, avg_temperature, median_temperature,
...
def compute_l3_cross_industry(db):
    ...
    for pref, et, s_type, s_min, edu, industry in rows:
        grp = emp_group(et)
    ...
    for (pref, band, edu_grp, grp), industry_counts in cells.items():
        ...
        # industry_count / total の逆数的な指標 → HHI逆数
...
def compute_a1_anomaly_stats(db):
    ...
    for metric in ANOMALY_METRICS:
        ...
        for pref, muni, et, val in rows:
            grp = emp_group(et)
            muni = muni or ""
            region_values[(pref, muni, grp)].append(float(val))
            # 都道府県集計
            region_values[(pref, "", grp)].append(float(val))
...
def compute_s1_cascade_summary(db):
    ...
    for pref, muni, industry, et, facility, s_min, s_type, emp_cnt, holidays, reason in rows:
        grp = emp_group(et)
        muni = muni or ""
        industry = industry or ""
        ...
        for key in keys:
            ...
            # 欠員補充判定（recruitment_reasonテキストに「欠員」を含む）
            if reason and "欠員" in str(reason):
                d["vacancy"] += 1
...
def verify(db):
    ...
    for r in db.execute("""
        ...
        ORDER BY emp_group
    """):
        print(f"    {r[0]}: n={r[1]}, 平均温度={r[2]:.3f}‰, 中央値={r[3]:.3f}‰, "
              f"緊急密度={r[4]:.3f}‰, 選択密度={r[5]:.3f}‰")

    # 温度トップ/ボトム都道府県
    print("  温度トップ5都道府県（正社員）:")
    for r in db.execute("""
        SELECT prefecture, avg_temperature, total_count
        FROM v2_text_temperature
        WHERE municipality='' AND industry_raw='' AND emp_group='正社員'
        ORDER BY avg_temperature DESC LIMIT 5
    """):
    ...
    for r in db.execute("""
        ...
        FROM v2_text_temperature
        WHERE municipality='' AND industry_raw='' AND emp_group='正社員'
        ORDER BY avg_temperature ASC LIMIT 5
    """):
        print(f"    {r[0]}: {r[1]:.3f}‰ (n={r[2]})")
