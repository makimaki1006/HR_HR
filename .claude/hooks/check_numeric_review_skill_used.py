...
# 2026-05-19 追加: 「全 PASS」「N issue 解消」「N cycle 完了」「全件解消」を追加
# (前日: 「Cycle 3 全 PASS」「全 issue 解消確認」等が hook を素通りした事故)
COMPLETION_RE = re.compile(
    ...
    r"|全\s*(件)?\s*(PASS|pass|✅|合格|OK|解消|解決|修正|完了)"
    r"|\d+\s*(つ|個|件)\s*(の)?\s*(問題|issue|バグ|Issue|不具合)\s*(全件|すべて|全)\s*(解消|解決|修正|完了)"
    r"|(Cycle|cycle|サイクル)\s*\d+\s*(全|完了|済|PASS|pass|✅)"
    r"|\d+\s*(cycle|サイクル)\s*(完了|済|終了|全件)"
    r"|(問題|issue|バグ|Issue|不具合)\s*(\d+)?\s*(つ|個|件)?\s*(すべて|全件|全部|全)\s*(解消|解決|修正|完了)"
    r"|本番\s*出力\s*(検証|確認)?\s*(済|完了|OK|PASS)"
    r"|button\.click\(\)?\s*経路\s*(検証|確認)\s*(完了|済|OK))",
)
