...
> ⚠️ **2026-05-15 重大事故由来 (絶対遵守ルール)**: テストで JS グローバル変数を直接 `window.X = ...` で set するのは禁止。実機ユーザーは UI 操作 (チェックボックス click、select change、button click) で値を更新する。**直接 set すると別 scope (例: DOMContentLoaded コールバック内のローカル var) の真因を見逃す**。

- [ ] **テストはユーザーが実際にやる操作と同じ手順で書く**:
  - select の値変更 → `.value = X` + `dispatchEvent('change')` ではなく option click 推奨
  - チェックボックス → `_selectedX = [...]` の直接代入 **絶対禁止**。`document.querySelector('#cb-id').click()` で UI 経路を通す
  - ボタン → 配下 onclick / openX JS を bypass せず、`button.click()` で実起動
  - dropdown/modal → 表示状態 (display/visibility/z-index) も含めて click 可能性を確認
- [ ] Playwright で UI 操作の各段階で **状態を log 確認**:
  - 「click した checkbox の value は何か」
  - 「click 後の関連グローバル変数 (`window.X`) の値は何か」
  - 「URL に期待されるクエリが付いたか」
- [ ] 「Playwright が動いているのに実機ユーザーには動かない」状況が報告されたら、**まず UI 経路を疑う**:
  - `var X` がローカル scope に閉じ込められて別 `<script>` から見えない
  - DOMContentLoaded / IIFE / モジュール scope の罠
  - 必ず `typeof window.X !== 'undefined'` で確認

**過去事故 (2026-05-15)**:
ユーザーが業界選択 → 反映されない問題。Playwright では `window._selectedIndustryRaws = [...]`
を直接 set していたため動いていたが、実機の checkbox 操作は DOMContentLoaded コールバック内
の var (window 未登録) を更新するだけで、別 `<script>` の openVariantReport JS から見えず、
URL に `&industry=` が付かなかった。Playwright が偽 OK を出し続けた。
...
❌ **🔴 Playwright/E2E で JS グローバル変数を `window.X = ...` で直接 set** (実機ユーザーは UI 操作で更新する。直接 set した状態は別 scope の罠 (var ローカル化など) を隠蔽し、偽 OK を量産する。**チェックボックス click / select change / button click 経由でテストする**)
