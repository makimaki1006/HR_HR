...
const OUT_DIR = path.resolve(__dirname, '..', '..', 'out', 'round8_p0_1_local');

test.describe.serial('Round 8 P0-1: occupation_segment_summary local PDF check', () => {
  test('Generate MI PDF locally (port 9217)', async ({ page, browserName, context }) => {
    ...
    const indGenCount = await miPage.locator('[data-mi-section="industry-gender"]').count();
    const quadCount = await miPage.locator('[data-mi-section="market-quadrant"]').count();
    console.log(`occupation-segment sections: ${segCount}`);
    console.log(`occupation-cells (legacy) sections: ${cellCount}`);
    console.log(`industry-gender sections (P0-2): ${indGenCount}`);
    console.log(`market-quadrant sections (P1-1): ${quadCount}`);
    if (quadCount > 0) {
      const qText = await miPage
        .locator('[data-mi-section="market-quadrant"]')
        .first()
        .innerText();
      console.log('=== MARKET-QUADRANT SECTION TEXT (first 1500 chars) ===');
      console.log(qText.slice(0, 1500));
    }
    ...
    if (indGenCount > 0) {
      const indText = await miPage
        .locator('[data-mi-section="industry-gender"]')
        .first()
        .innerText();
      console.log('=== INDUSTRY-GENDER SECTION TEXT (first 1200 chars) ===');
    ...
    const pdfPath = path.join(OUT_DIR, 'mi_local.pdf');
    await miPage.pdf({
      path: pdfPath,
      format: 'A4',
