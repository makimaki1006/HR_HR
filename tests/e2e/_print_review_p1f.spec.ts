/**
 * tests/e2e/_print_review_p1f.spec.ts
 *
 * P1f (2026-05-06): commit ad6488d 後の本番 PDF 再生成 (アクションバー経由)。
 *  - render_mi_salary_living_cost (給与・生活コスト比較表) を自治体単位で集約済
 *  - 「代表職種」列 + 「ほか N 職種」併記
 *  - p1e (commit 556d960 / 印刷 CSS 修正) は維持されている前提
 *  - 出力: out/print_review_p1f/mi_via_action_bar.pdf
 ...
 *   npx playwright test _print_review_p1f --project=chromium
 */
...
const OUT_DIR = path.resolve(__dirname, '..', '..', 'out', 'print_review_p1f');

test.describe.serial('Print review p1f (commit ad6488d / living cost aggregation)', () => {
