...
- **page 19 注釈統合誤判断**: 文字数削減効果を期待した修正が実際には別ターゲットだった (commit `9a616f9`)
- **PDF 余白計測誤認**: フッター margin box を本文最下端として誤計測し「4mm → 12mm 改善」を誤認 (`docs/PDF_BOTTOM_MARGIN_ROOT_CAUSE_INVESTIGATION.md`)
- **ヒストグラム軸ラベル重なり (page 5/6)**: 自動検査では検出されず、目視でのみ確認可能 (`ROUND6_PRINT_PDF_DELIVERY_QUALITY.md` §5 P2-C)
...
- 例: 注釈統合 (commit `9a616f9`) で「page 19 が空白になっていないか」だけ確認し、page 18 / 20 の章割れを見落とすパターン。
- §1.1 の通り「全ページ目視」が必須。

### 5.5 `text block 抽出だけで page 内容判定` (Round 6 教訓: page 19 注釈統合誤判断)
