...
- 実行日時 (UTC): 2026-05-05T17:34:59.841337+00:00 〜 2026-05-05T17:34:59.875759+00:00
- 実行モード: `local`
- 検証者: Worker P0 (READ-ONLY)

## 0. 結論

**ローカル PASS** (3/3)。Turso 実行は環境変数未設定でスキップ。Turso 結果は未取得 (ユーザー手動実行待ち)。
...
(Turso 未実行 — env 未設定または `--local` モード)
...
- ローカルのみ実行。Turso 互換性は手動再実行 (`--both`) で確認要。
...
生成: `scripts/verify_turso_window_function.py` (2026-05-05 17:34 UTC)